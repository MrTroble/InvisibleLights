...
public class BlockCustomState extends BlockCustomLight {
    ...
    private void updatePowerState(final IBlockState state, final World worldIn,
            final BlockPos pos) {
        if (worldIn.isRemote)
            return;
    ...
    @Override
    public void tick(final IBlockState state, final World worldIn, final BlockPos pos,
            final Random rand) {
        if (worldIn.isRemote)
            return;
